...
- 最终交付前必须做自主审核，产出 `self-review.md`。审核要对照正式协议/合同/测试协议等参考资料检查事实冲突，对照模板检查格式偏离，对照导出的图和 DOCX 预览检查框图/拓扑图是否显示完整。
...
- `self-review.md` 已完成，且没有未解决的 Blocker 问题。
...
自动校验通过后，还要由 Agent 自主复核并写入 `self-review.md`：

- 事实一致性：项目名称、单位、合同范围、交付物、数量、日期、验收指标、部署位置、测试方法、质保/服务承诺、结论等是否和协议/合同/测试协议一致。
- 支撑充分性：通过/符合/完成/无问题等结论是否有来源依据；没有来源时是否使用了保守表述并写入缺失说明。
- 模板符合性：章节顺序、标题、表格形状、签字区、页眉页脚、TOC/字段、字体、页边距、模板填写要求是否被破坏。
- 图形显示：结构图、流程图、组成图、拓扑图、架构图等是否可读、未裁切、未超出版心、插入位置正确。
- 残留内容：`XXX`、`××`、`202X`、`……`、旧项目名、旧结论、示例说明是否清理。
- 编号引用：图表编号、正文中的“见表/如图”引用是否对应正确。

发现事实冲突、模板结构破坏、必需图显示不全、DOCX 损坏、最终结论无来源支撑等 Blocker 时，不能交付最终版；应先修复，或明确请求用户补充/确认。
