...
For direct Claude Code CLI invocation, use OpenClaw's `exec` tool in the workspace:

### Basic Commands

```bash
# One-off query (print mode) - recommended for OpenClaw
...
claude -r "session-name" "continue the task"
```
...
|------|-------------|---------|
| `-p` | Print mode (one-off query) | `claude -p "query"` |
| `-c` | Continue last session | `claude -c` |
| `-r` | Resume session | `claude -r "name" "query"` |
| `--model` | Specify model | `claude --model sonnet "query"` |
| `--max-turns` | Limit turns | `claude -p --max-turns 3 "query"` |
| `--dangerously-skip-permissions` | Skip prompts | `claude -p --dangerously-skip-permissions "query"` |
...
# Bug fix
claude -p "fix the null pointer exception in userService.js"

# Feature development
claude -p "add user authentication with JWT tokens"

# Explain code
claude -p "explain what this function does in detail"
```
...
}' -p "review the changes"
```

**Note:** Claude Code must be installed (`curl -fsSL https://claude.ai/install.sh | bash`) and authenticated before using CLI commands.
